...
Two `std::map`s of price levels (bids descending via `std::greater<>`, asks
ascending via `std::less<>`) plus an `unordered_map<OrderID, Locator>` for
...
```
bids_ (sorted high to low)       asks_ (sorted low to high)
...
not invalidate iterators to other nodes. That property is what makes the
stored iterator in the locator safe to use for the lifetime of the order.
When a cancel comes in we look up the locator, jump straight to the list
node, and erase it without scanning the level.
